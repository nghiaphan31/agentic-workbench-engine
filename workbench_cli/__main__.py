...
Commands:
  ...
  workbench-cli.py start-feature --req-id <REQ-NNN> [ --slug <slug> ]  — Transition INIT/MERGED → STAGE_1_ACTIVE
  workbench-cli.py lock-requirements --req-id <REQ-NNN>  — Transition STAGE_1_ACTIVE → REQUIREMENTS_LOCKED
  workbench-cli.py set-red --req-id <REQ-NNN>       — Transition REQUIREMENTS_LOCKED → RED
  workbench-cli.py review-pending --req-id <REQ-NNN> — Transition GREEN → REVIEW_PENDING
  workbench-cli.py merge --req-id <REQ-NNN>        — Mark feature as MERGED, close pipeline cycle
  workbench-cli.py --version / --cli-version       — Display CLI version and exit
...
def _is_git_repo(repo_path):
    """Check if path is a git repo, handling both regular repos and submodules.
...
def cmd_upgrade(version):
    ...
    # Write UPGRADE_IN_PROGRESS state
    ...
    # Restore INIT state ( Arbiter-owned, reset after upgrade)
    state = load_state_json(repo_path)
    state["state"] = "INIT"
    state["last_updated"] = datetime.now(timezone.utc).isoformat()
    state["last_updated_by"] = "workbench-cli"
...
def cmd_merge(req_id):
    ...
    state["last_updated_by"] = "workbench-cli"
    _write_state(repo_path, state)
    print(f"[WORKBENCH-CLI] {req_id} — state = MERGED. Pipeline cycle complete.")
    print(f"  Next: Run workbench-cli.py start-feature to begin a new feature")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="workbench-cli",
        description="Agentic Workbench v2.1 Bootstrapper — Deterministic project scaffolding and pipeline management"
    )
    ...
    start_parser = subparsers.add_parser("start-feature", help="Transition INIT/MERGED → STAGE_1_ACTIVE")
    start_parser.add_argument("--req-id", required=True, help="Requirement ID (e.g., REQ-001)")
    start_parser.add_argument("--slug", help="Optional branch slug suffix")

    # lock-requirements command
    lock_parser = subparsers.add_parser("lock-requirements", help="Transition STAGE_1_ACTIVE → REQUIREMENTS_LOCKED")
    lock_parser.add_argument("--req-id", required=True, help="Requirement ID")

    # set-red command
    red_parser = subparsers.add_parser("set-red", help="Transition REQUIREMENTS_LOCKED → RED")
    red_parser.add_argument("--req-id", required=True, help="Requirement ID")

    # review-pending command
    review_parser = subparsers.add_parser("review-pending", help="Transition GREEN → REVIEW_PENDING")
    review_parser.add_argument("--req-id", required=True, help="Requirement ID")

    # merge command
    merge_parser = subparsers.add_parser("merge", help="Mark feature as MERGED")
    merge_parser.add_argument("--req-id", required=True, help="Requirement ID")

    # version flag
    parser.add_argument("--version", action="store_true", help="Display CLI version and exit")
    parser.add_argument("--cli-version", action="store_true", help="Display CLI version and exit")

    args = parser.parse_args()

    # Handle version flag
    if args.version or args.cli_version:
        print(f"workbench-cli {load_template_version()}")
        sys.exit(0)
